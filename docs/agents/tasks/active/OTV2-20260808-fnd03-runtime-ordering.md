...
updated_at: 2026-08-08T19:06:00+02:00
...
Persist the owner-directed FND-03 runtime-execution analysis as a bounded architecture package and continue resolving only the runtime invariants that genuinely block the later FND-03 contract. The package must define a safe authoritative execution model for ChannelRuntime and InstanceRuntime without implementing the runtime or prematurely choosing benchmark-sensitive libraries, worker counts, tick rates, orchestration products or persistence technology.
...
- **DERIVED:** CommandId ordering, simulation execution order and wall-clock time must remain distinct concepts.
...
- [ ] Persist the current owner-directed runtime-execution conclusions in one bounded architecture baseline.
- [ ] Separate CommandId order, authoritative simulation commit order, monotonic deadlines and wall-clock timestamps explicitly.
- [ ] Define one logical authoritative execution model usable by ChannelRuntime and InstanceRuntime.
- [ ] Define auxiliary-work proposal/stale-result rules without allowing worker threads to mutate authoritative state directly.
- [ ] Define ownership-generation and NodeId separation for crash/replacement safety.
- [ ] Define queue/backpressure classes and fail-open/fail-closed principles without guessing benchmark-sensitive numeric capacities.
- [ ] Analyse the deterministic ordering rule for concurrent commands, timers, worker completions and system/runtime events.
- [ ] Apply the mandatory architecture decision timing test to every material recommendation.
- [ ] Keep runtime implementation, library selection and production activation explicitly unauthorized.
...
Task created from current `main` after the reconnect-protection package and its archive were merged. The first persisted architecture artifact will capture the runtime execution model already analysed with the owner, then continue into deterministic event ordering and overload/failure semantics.
...
```yaml
last_progress: Created the bounded FND-03 runtime-ordering task from current main after confirming no open PRs and no predecessor delivery blocker.
status: investigating
branch: docs/OTV2-20260808-fnd03-runtime-ordering
head_sha: null
pr: null
...
blocker: null
next_action: Persist the authoritative runtime execution analysis baseline and continue resolving deterministic commit ordering.
```
